...
Vue3 + Vite + Vue Router SPA 应用，演示 PKCE 完整流程：

- 公共客户端 (`client_id: spa-client-vue3`)，无 client_secret，认证方式为 `NONE`
- 手动实现 PKCE：生成 `code_verifier` / `code_challenge`，SHA-256 哈希
- 授权码换取令牌时携带 `code_verifier`
- **无 `refresh_token`**：公共客户端不支持刷新令牌
- 支持 Silent Refresh（`prompt=none`）静默重新授权
- Vue Router 导航守卫：未认证访问受保护页自动跳登录，已认证访问首页跳测试页
- API 401 拦截器：Token 无效或过期自动清除 session
- 支持 OIDC 登出
- 调用 Resource Server 获取受保护数据
...
| `/profile` | 测试页（用户信息、Token、API 调用、登出） | 需认证 |
...
用于**用户长期在线的应用**，避免因 access_token 过期（通常 1 小时）强制重新登录。两种机制：

| 机制 | 适用客户端 | 原理 |
|------|-----------|------|
| **refresh_token** | 机密客户端（有 client_secret） | 用 refresh_token 换新 access_token，无需用户交互 |
| **Silent Refresh** | 公共客户端（SPA，无 client_secret） | 隐藏 iframe + `prompt=none` 静默重新授权 |
...
#### spa-client-vue3（端口 3001）— Silent Refresh 自动续期

SPA 公共客户端无 refresh_token（Spring Authorization Server 默认不向 `ClientAuthenticationMethod.NONE` 客户端颁发），需要用 Silent Refresh 续期：

```
access_token 过期 → 定时检测 isAuthenticated()
  → iframe 加载 /oauth2/authorize?prompt=none
  → 用户已登录 → 自动签发新授权码
  → exchangeCode() 获取新 access_token
  → 用户无感知
```

> **注意**：Silent Refresh 需要用户在授权服务器仍有有效 session，否则 iframe 会显示登录页，此时应回退到完整登录流程。
